...
public class BytesTerminal : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        angold = 0;
        point = Vector3.one;
        InvokeRepeating("ontime", 0, 0.01f);
    ...
    void ontime()
    {
        ...
        Angletext.text =
            ...
            "\n_connectButtonStatus=" + _connectButtonStatus.ToString() +
            "\n_lastNameEnteredToConnect=" + _lastNameEnteredToConnect.ToString() +
            "\ndeviceNameInput.text=" + deviceNameInput.text +
            "\nifcal=" + ifcal.ToString() +
            "\nfullyCalibrated=" + fullyCalibrated.ToString() +
            "\ndis_count=" + dis_count.ToString() +
            "\nsent_count" + sent_count.ToString()+
            "\nBtConnector.available()=" + BtConnector.available().ToString() +
            "\nsensor=" + sensor.ToString() +
            "\nsema=" + sema.ToString()+
            "\ncurrentAngle="+ currentAngle.ToString();
        */
        ...
        if (BtConnector.isConnected())
        {
            ...
            if (dis_count == 100)
            {
                Connect();
                dis_count = -1;
                Angletext.text += "OOPS!\nwe'r disconnected from " + deviceNameInput.text + ", wait for a few seconds...";
            }
